...
TargetDetector::TargetDetector()
{
    ...
    int ret = NXU_DFD_Create(&hDFD);
    // int ret = NXU_DFD_CreateWithModel(&hDFD, "dfd.weights");
    if (ret == NXU_SUCCESS)
    {
        hProcessID = hDFD;
    }
    ...
    {
        throw std::runtime_error("Failed to initialize DFD handler");
    }
}
...
TargetDetector::~TargetDetector()
{
    ...
    {
        // 假设 nxu_release_dfd 是用于释放目标检测句柄的函数
        NXU_DFD_Destroy(hProcessID);
        hProcessID = nullptr;
    }
}
...
bool TargetDetector::process_image(const std::string &input_path)
{
    ...
    // 处理图像
    int ret = NXU_DFD_Process(hProcessID, &stImg);
    if (ret != NXU_SUCCESS)
    {
        return false;
    }
    ...
    // 获取结果并遍历目标
    NXU_DFD_Result_S stResults;
    ret = NXU_DFD_GetResults(hProcessID, &stResults);
    if (ret != NXU_SUCCESS)
    {
        return false;
    }
    ...
    {
        NXU_DFD_TgtItem_S stItem = stResults.astTgtItems[i];
