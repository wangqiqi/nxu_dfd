...
namespace nxu_dfd_utils
{
    /* 构造函数 */
    CNXUDFDUtils::CNXUDFDUtils()
    {
        /* 清理目标信息 */
        __clean_objs();

        /* 初始化模型，加载模型参数 */
        m_PCYOLOV8DetNet = new CYOLOV8_ORT_Detector(DFD_MODEL_DEFAULT);
    }

    /* 用路径构造初始化模型 */
    CNXUDFDUtils::CNXUDFDUtils(const std::string &modelPath)
    {
        /* 清理目标信息 */
        __clean_objs();
    ...
    /* 清理目标信息 */
    void CNXUDFDUtils::__clean_objs()
    {
        m_vecObjs.clear();
    ...
    // DONE
    NXU_STATUS_CODE_E CNXUDFDUtils::process(const NXU_IMAGE_S *pstImage)
    {
        int ret;
        ...
        //     std::cout << "    ++++++  In CNXUDFDUtils----Process Convert Image Time Span ++++++ " << (dtEnd - dtStart) << " ms" << std::endl;
        //     dtStart = __get_current_time_proc();
        // #endif // SHOW_DEBUG_TIME
        ...
        // #ifdef SHOW_DEBUG_TIME
        //     dtEnd = __get_current_time_proc();
        //     std::cout << "    ++++++  In CNXUDFDUtils----Scale Image Time Span ++++++ " << (dtEnd - dtStart) << " ms" << std::endl;
        //     dtStart = __get_current_time_proc();
        // #endif // SHOW_DEBUG_TIME
        ...
        // #ifdef SHOW_DEBUG_TIME
        //     dtEnd = __get_current_time_proc();
        //     std::cout << "++++++  In CNXUDFDUtils----Network Process Time Span ++++++ " << (dtEnd - dtStart) << " ms" << std::endl;
        //     dtStart = __get_current_time_proc();
        // #endif // SHOW_DEBUG_TIME

        NXU_RECT_S stBB;
        NXU_S32 s32TargetNum = m_vecObjs.size();
        if (s32TargetNum > MAX_DFD_TGT_NUM)
        {
            s32TargetNum = MAX_DFD_TGT_NUM;
        }
        ...
        for (int i = 0; i < s32TargetNum; i++)
        {
            ...
            // 类型不知道, 需要进行后处理判断
            stItem.enTgtType = (NXU_DFD_TgtType_E)(obj.s32labelID);
        ...
        // #ifdef SHOW_DEBUG_TIME
        //     dtEnd = __get_current_time_proc();
        //     std::cout << "    ++++++  In CNXUDFDUtils----Postprocess Time Span ++++++ " << (dtEnd - dtStart) << " ms" << std::endl;
        // #endif // SHOW_DEBUG_TIME
    ...
    // 后处理，将目标转换为事件
    NXU_STATUS_CODE_E CNXUDFDUtils::__post_process()
    {
        // TODO: 后处理逻辑
        // 将目标分离到不同类型的目标集合里
    ...
    // 获得结果
    NXU_STATUS_CODE_E CNXUDFDUtils::get_results(NXU_DFD_Result_S *pstResult)
    {
        if (NULL == pstResult)
        {
            return NXU_FAILURE;
        ...
        // 将内部结果拷贝出去
        memcpy(pstResult, &m_stResults, sizeof(NXU_DFD_Result_S));

        return NXU_SUCCESS;
    }

    // 将图像转到指定大小
    void CNXUDFDUtils::__scaleIm(cv::Mat &src, cv::Mat &dst, int s32DesignedWH, float *scale)
    {
